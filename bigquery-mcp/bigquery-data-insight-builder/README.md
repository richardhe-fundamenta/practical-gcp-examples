...
export DATASTORE_DB='ai-agents-db'

# Install dependencies
uv sync

# Create the Datastore database (if not already created)
gcloud firestore databases create --database=${DATASTORE_DB} \
    --location=${REGION} \
    --type=datastore-mode \
    --project=$PROJECT_ID

# Create required Cloud Datastore indexes
gcloud datastore indexes create index.yaml --project=$PROJECT_ID --database=${DATASTORE_DB}

# Wait for indexes to be created (check status)
gcloud datastore indexes list --project=$PROJECT_ID --database=${DATASTORE_DB}
