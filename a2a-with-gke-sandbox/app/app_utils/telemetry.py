...
def setup_telemetry() -> str | None:
    ...
    if bucket and capture_content != "false":
        ...
        os.environ.setdefault(
            ...
            f"service.namespace=a2a-with-gke-sandbox,service.version={commit_sha}",
        )
    ...
    # get_gcp_resource sets service.name from GOOGLE_CLOUD_AGENT_ENGINE_ID, which is empty on
    # Cloud Run — leaving the trace service name blank. Merge a real service.name in (the
    # merge argument wins on conflict).
    otel_resource = get_gcp_resource(project_id).merge(
        Resource.create(
            {"service.name": os.environ.get("OTEL_SERVICE_NAME", "a2a-with-gke-sandbox")}
        )
    )
