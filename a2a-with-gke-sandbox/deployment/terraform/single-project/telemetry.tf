...
resource "google_bigquery_table" "genai_logs_table" {
  ...
  # Cloud Logging auto-evolves this sink table's schema (adds httpRequest, operation,
  # apphub*, extra labels, etc. as it sees new fields). Don't let Terraform revert that —
  # reverting removes columns, which forces a destructive table replace and drops logged rows.
  lifecycle {
